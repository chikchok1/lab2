...
typedef struct {
    ...
    char room[MAXROOM];     // 방 이름
    int registered;         // 0: 접속 직후, 1: /join 완료
...
void broadcast_to_room(ServerContext *server, int sender_idx, const char *msg) {
    ClientContext *sender = &server->clients[sender_idx];
    char packet[MAXBUF];

    // 메시지 포맷팅: [room 이름][닉네임] 메시지
    snprintf(packet, sizeof(packet), "[room %s][%s] %s\n", 
             sender->room, sender->nickname, msg);
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        ...
        // 전송 (실패 시 로그만 출력)
        if (send(target->fd, packet, strlen(packet), 0) == -1) {
            perror("send broadcast");
        }
    }
}

/* 완성된 한 줄의 명령어 처리 (/join, /msg) */
void process_command(ServerContext *server, int idx, char *line) {
    ...
    // 2. /msg <message>
        ...
        if (*msg == '\0') {
            send(fd, "ERR Empty message\n", 18, 0);
            return;
        }

        broadcast_to_room(server, idx, msg);
    }
    else {
...
void handle_client_data(ServerContext *server, int idx) {
    ...
    // 버퍼 오버플로우 방지 (너무 길면 초기화)
    if (cli->cmd_len + nbytes >= MAXBUF) {
        cli->cmd_len = 0; 
    }
    
    // 수신된 데이터를 클라이언트 전용 버퍼에 추가
    memcpy(cli->cmd_buf + cli->cmd_len, buf, nbytes);
    cli->cmd_len += nbytes;
    cli->cmd_buf[cli->cmd_len] = '\0'; // 문자열 끝 보장
    ...
    while ((p = strchr(cli->cmd_buf, '\n')) != NULL) {
        *p = '\0'; // 개행을 null로 변경하여 문자열 분리
        ...
        // 처리한 명령어 뒤의 남은 데이터를 버퍼 앞으로 당기기 (memmove)
        int processed_len = (p - cli->cmd_buf) + 1; // 명령어 길이 + null문자(원래 \n)
